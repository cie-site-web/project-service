...
export class CreateTechnologyValidator {
  validate(command: CreateTechnologyCommand): void {
    if (!NANOID_REGEX.test(command.architectureId)) {
      throw new BusinessError(CodesError.TECHNOLOGY_ARCHITECTURE_ID_INVALID);
    }
    if (!NANOID_REGEX.test(command.frameworkId)) {
      throw new BusinessError(CodesError.TECHNOLOGY_FRAMEWORK_ID_INVALID);
    }
    if (!NANOID_REGEX.test(command.utilsGestionId)) {
